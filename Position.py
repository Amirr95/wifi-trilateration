# TODO Change it so only APs with SSID=Bontech are considered.
...
print("S.No", '       ', "SSID", '      ', "RSSI(dBm)", '      ', "Freq", '      ', "Channel", '     ', "MAC",
      '         ', "Distance(mts)")
print("here")
...
d3 = (x3 ** 2) + (y3 ** 2) - (float(dict_wifi[ind[2]][5]) ** 2)
plt.plot(x1, y1, 'ro')
plt.annotate(dict_wifi[ind[0]][0] + '\n' + str(round(float(dict_wifi[ind[0]][5]), 2)) + 'mts', xy=(x1, y1),
             xycoords='data', xytext=(-30, +40), textcoords='offset points', fontsize=10,
             arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=.2"))
plt.plot(x2, y2, 'ro')
plt.annotate(dict_wifi[ind[1]][0] + '\n' + str(round(float(dict_wifi[ind[1]][5]), 2)) + 'mts', xy=(x2, y2),
             xycoords='data', xytext=(+30, +40), textcoords='offset points', fontsize=10,
             arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=.2"))
plt.plot(x3, y3, 'ro')
plt.annotate(dict_wifi[ind[2]][0] + '\n' + str(round(float(dict_wifi[ind[2]][5]), 2)) + 'mts', xy=(x3, y3),
             xycoords='data', xytext=(+30, -40), textcoords='offset points', fontsize=10,
             arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=.2"))
